...
from harness import Session, Scope, Memory
...
_memory = Memory(base_dir=str(SKILL_DIR / "logs"))

# Registry for dynamically created functions
_dynamic_functions: dict[str, dict] = {}


def _create_worker(model: str = None, scope: Scope = None) -> Session:
    ...
    worker = ClaudeCodeSession(model=model or DEFAULT_WORKER_MODEL, max_turns=1)
    if scope:
        worker.apply_scope(scope, {})
    return worker
...
def observe(programmer: Session, task: str, app_name: str = None,
    ...
    """
    # Memory: start run if needed
    run_active = _memory._run_id is not None
    if not run_active:
        _memory.start_run(task=f"observe: {task}")

    call_id = _memory.log_function_call(
        "observe",
        params={"task": task, "app_name": app_name},
        scope="isolated",
        session_type="ClaudeCodeSession",
    )
    ...
    try:
        ...
        if _memory._run_id:
            _memory.save_media("screenshot.png", source_path=shot.path)
        ...
        worker = _create_worker(worker_model, scope=Scope.isolated())
...
{json.dumps(ObserveResult.model_json_schema(), indent=2)}"""
        ...
        if not run_active:
            _memory.end_run(status="success")
...
def learn(programmer: Session, app_name: str,
    ...
    run_active = _memory._run_id is not None
    if not run_active:
        _memory.start_run(task=f"learn: {app_name}")
...
def act(programmer: Session, action: str, target: str,
    ...
    run_active = _memory._run_id is not None
    if not run_active:
        _memory.start_run(task=f"act: {action} on {target}")

    call_id = _memory.log_function_call(
        "act",
        params={"action": action, "target": target, "text": text, "app_name": app_name},
        scope="isolated",
        session_type="ClaudeCodeSession",
    )
    ...
    try:
        ...
        if _memory._run_id:
            _memory.save_media("before_screenshot.png", source_path=before_shot.path)
...
{json.dumps(ActResult.model_json_schema(), indent=2)}"""
        ...
        try:
            ...
            if result.success and result.coordinates:
                ...
                if _memory._run_id:
                    _memory.save_media("after_screenshot.png", source_path=after_shot.path)
...
def remember(programmer: Session, operation: str, app_name: str,
    ...
    run_active = _memory._run_id is not None
    if not run_active:
        _memory.start_run(task=f"remember: {operation} for {app_name}")

    call_id = _memory.log_function_call(
        "remember",
        params={"operation": operation, "app_name": app_name, "details": details},
        scope="isolated",
        session_type="ClaudeCodeSession",
    )
...
def navigate(programmer: Session, target_state: str, app_name: str,
    ...
    run_active = _memory._run_id is not None
    if not run_active:
        _memory.start_run(task=f"navigate: {target_state} in {app_name}")

    call_id = _memory.log_function_call(
        "navigate",
        params={"target_state": target_state, "app_name": app_name},
        scope="isolated",
        session_type="ClaudeCodeSession",
    )
...
def verify(programmer: Session, expected: str,
    ...
    run_active = _memory._run_id is not None
    if not run_active:
        _memory.start_run(task=f"verify: {expected}")
...
def meta_create(name: str, docstring: str, params: dict, returns: dict,
                worker_model: str = None) -> dict:
    """Meta Agentic Function — creates new Agentic Functions at runtime.

    The created function will:
    1. Gather data via Python Runtime (screenshot + OCR if needed)
    2. Send the docstring as prompt to a Worker Session
    3. Parse the structured response
    4. Register as an MCP tool (if MCP server is running)

    Args:
        name: Function name (snake_case)
        docstring: The prompt/instructions for the LLM
    ...
    # Build return type dynamically
    field_defs = {}
    for field_name, field_desc in returns.items():
        if "int" in field_desc.lower():
            field_defs[field_name] = (int, 0)
        elif "float" in field_desc.lower():
            field_defs[field_name] = (float, 0.0)
        elif "bool" in field_desc.lower():
            field_defs[field_name] = (bool, False)
        elif "list" in field_desc.lower():
            field_defs[field_name] = (list, [])
        elif "dict" in field_desc.lower():
            field_defs[field_name] = (dict, {})
        else:
            field_defs[field_name] = (str, "")

    ReturnModel = create_model(f"{name}_result", **field_defs)

    def dynamic_fn(programmer: Session = None, worker_model_override: str = None, **kwargs) -> dict:
        """Dynamically created Agentic Function."""
        model = worker_model_override or worker_model

        run_active = _memory._run_id is not None
        if not run_active:
            _memory.start_run(task=f"{name}: {kwargs}")

        call_id = _memory.log_function_call(
            name, params=kwargs, scope="isolated",
            session_type="ClaudeCodeSession",
        )

        start_time = time.time()

        # Gather screen data if docstring mentions screen/GUI/UI
        context_data = {}
        needs_screen = any(kw in docstring.lower() for kw in
                          ["screen", "gui", "ui", "click", "button", "window", "app"])

        if needs_screen:
            shot = take_screenshot()
            ocr = run_ocr(shot.path)
            context_data["screenshot_path"] = shot.path
            context_data["ocr_texts"] = [t.get("label", "") for t in ocr.texts[:30]]
        ...
        if param_lines:
            prompt_parts.extend(["### Input", param_lines, ""])
        if context_data.get("ocr_texts"):
            prompt_parts.extend([
                "### Screen OCR",
                "\n".join(f"  '{t}'" for t in context_data["ocr_texts"]),
                "",
            ])
        prompt_parts.extend([
            "### Output format",
            "Return ONLY valid JSON matching this schema:",
            json.dumps(ReturnModel.model_json_schema(), indent=2),
        ])

        prompt = "\n".join(prompt_parts)

        worker = _create_worker(model, scope=Scope.isolated())

        if context_data.get("screenshot_path"):
            reply = worker.send({"text": prompt, "images": [context_data["screenshot_path"]]})
        else:
            reply = worker.send(prompt)

        try:
            data = _parse_json(reply)
            result_obj = ReturnModel(**data)
            result = result_obj.model_dump()
        except Exception:
            result = {"raw_response": reply[:500]}

        duration_ms = (time.time() - start_time) * 1000
        _memory.log_function_return(name, result=result, status="success", duration_ms=duration_ms)
        ...
        if programmer is not None:
            try:
                _report_to_programmer(
                    programmer, name,
                    ReturnModel(**result) if isinstance(result, dict) and "raw_response" not in result
                    else type("R", (BaseModel,), {"__annotations__": {"details": str}})(**{"details": str(result)})
                )
            except Exception:
                pass
    ...
    return {
        "functions": {
            name: {
                ...
                "returns": list(info["returns"].keys()),
            }
            for name, info in _dynamic_functions.items()
        },
        "count": len(_dynamic_functions),
    }


def meta_call(name: str, programmer: Session = None, **kwargs) -> dict:
    """Call a dynamically created Agentic Function by name."""
    if name not in _dynamic_functions:
        return {"error": f"Function '{name}' not found. Available: {list(_dynamic_functions.keys())}"}

    fn = _dynamic_functions[name]["fn"]
    return fn(programmer=programmer, **kwargs)
